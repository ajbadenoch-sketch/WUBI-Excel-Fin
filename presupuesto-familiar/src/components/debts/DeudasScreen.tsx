...
import { useDeudas, useTarjetas, useMSI, useMovimientos, useConfig } from '@/lib/hooks';
...
function OverviewTab({ deudas, movimientos, config }: { deudas: any[]; movimientos: any[]; config: any }) {
  ...
  const totalInterestPortion = deudas.reduce((s, d) => s + d.saldoActual * (d.tasaInteres / 12), 0);
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="bg-gray-800 rounded-xl p-3 text-center">
          ...
          <p className={`text-sm font-bold ${ratioStatus.color}`}>
            {ratioStatus.icon} {(ratio * 100).toFixed(0)}%
          </p>
        </div>
      </div>
          ...
          <div key={d.id} className="bg-gray-800 rounded-xl p-3">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-gray-400">{d.tipo} • {(d.tasaInteres * 100).toFixed(1)}% anual</p>
              </div>
              <span className={`px-2 py-0.5 text-xs rounded-full ${
                d.estado === 'Liquidada' ? 'bg-emerald-900/50 text-emerald-300' : 'bg-red-900/50 text-red-300'
              }`}>
                {d.estado}
              </span>
            </div>
            ...
            <div className="w-full bg-gray-700 rounded-full h-1.5 mt-2">
              <div
                ...
                style={{ width: `${Math.max(0, ((d.montoOriginal - d.saldoActual) / d.montoOriginal) * 100)}%` }}
              />
            </div>
            <p className="text-xs text-gray-500 mt-1">
              {((((d.montoOriginal - d.saldoActual) / d.montoOriginal) * 100) || 0).toFixed(0)}% pagado
            </p>
          </div>
        ))
      )}
...
function StrategyTab({ deudas }: { deudas: any[] }) {
  ...
  const avalanche = sortAvalanche(activeDebts.map((d) => ({
    nombre: d.nombre, saldo: d.saldoActual, tasaInteres: d.tasaInteres, pagoMinimo: d.pagoMinimo, pagoReal: d.pagoReal,
  })));
  const snowball = sortSnowball(activeDebts.map((d) => ({
    nombre: d.nombre, saldo: d.saldoActual, tasaInteres: d.tasaInteres, pagoMinimo: d.pagoMinimo, pagoReal: d.pagoReal,
  })));
...
function StrategyList({ title, items, subtitle }: { title: string; items: any[]; subtitle: string }) {
  return (
    <div className="bg-gray-800 rounded-xl p-3">
      ...
      {items.length === 0 ? (
        <p className="text-xs text-gray-500">Sin deudas activas</p>
      ) : (
        items.map((d, i) => (
          <div key={d.nombre} className="flex items-center gap-2 py-1.5 border-t border-gray-700 first:border-0">
            ...
            <div className="flex-1">
              <p className="text-xs text-gray-200">{d.nombre}</p>
              <p className="text-xs text-gray-500">{(d.tasaInteres * 100).toFixed(1)}% • {formatMXN(d.saldo)}</p>
            </div>
...
function AmortizationTab({ deudas }: { deudas: any[] }) {
  ...
  const schedule = deuda
    ? generateAmortization(deuda.saldoActual, deuda.tasaInteres, deuda.pagoReal, new Date(deuda.fechaInicio))
    : [];
  ...
  return (
    <div className="space-y-3">
      ...
      {schedule.length === 0 ? (
        <div className="bg-gray-800 rounded-xl p-6 text-center">
          <p className="text-sm text-gray-400">Selecciona una deuda para ver la tabla</p>
        </div>
      ) : (
        <div className="bg-gray-800 rounded-xl p-3 overflow-x-auto">
            ...
            <tbody>
              {schedule.slice(0, 60).map((row) => (
                <tr key={row.month} className="text-gray-300 border-t border-gray-700">
                  ...
                  <td className="py-1">{format(row.date, 'MMM yy', { locale: es })}</td>
                  <td className="py-1 text-right">{formatMXN(row.startingBalance)}</td>
                  <td className="py-1 text-right">{formatMXN(row.payment)}</td>
                  <td className="py-1 text-right text-red-400">{formatMXN(row.interest)}</td>
                  <td className="py-1 text-right text-emerald-400">{formatMXN(row.principal)}</td>
          ...
          {schedule.length > 60 && (
            <p className="text-xs text-gray-500 mt-2 text-center">
              Mostrando 60 de {schedule.length} meses
            </p>
          )}
        </div>
      )}
      ...
      {tarjetas.map((card) => {
        ...
        return (
          <div key={card.id} className="bg-gray-800 rounded-xl p-3 space-y-3">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-sm font-medium text-white">{card.nombre}</p>
                <p className="text-xs text-gray-400">{card.banco}</p>
              </div>
              <span className={`text-xs font-medium ${utilStatus.color}`}>
                {(utilization * 100).toFixed(0)}% Uso
              </span>
            </div>
            ...
            <div className="w-full bg-gray-700 rounded-full h-3">
              <div
                className={`h-3 rounded-full transition-all ${
                  utilization < 0.3 ? 'bg-emerald-500' : utilization <= 0.7 ? 'bg-yellow-500' : 'bg-red-500'
                }`}
                style={{ width: `${Math.min(utilization * 100, 100)}%` }}
              />
            </div>
            <div className="flex justify-between text-xs text-gray-400">
              <span>Saldo: {formatMXN(card.saldoActual)}</span>
              <span>Límite: {formatMXN(card.limiteCredito)}</span>
            </div>
  ...
  if (msiList.length === 0) {
    return (
      <div className="bg-gray-800 rounded-xl p-6 text-center">
        ...
        <p className="text-sm text-gray-400">No hay compras MSI activas</p>
      </div>
    );
  }
          ...
          <div key={m.id} className="bg-gray-800 rounded-xl p-3">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-gray-400">{m.tienda} • {m.tarjeta}</p>
              </div>
              <span className={`px-2 py-0.5 text-xs rounded-full ${
                m.estado === 'Activo' ? 'bg-blue-900/50 text-blue-300' : 'bg-emerald-900/50 text-emerald-300'
              }`}>
                {m.estado}
              </span>
            </div>
            <div className="flex justify-between text-xs mt-2 text-gray-400">
              <span>Total: {formatMXN(m.montoTotal)}</span>
              <span>Mensual: {formatMXN(m.pagoMensual)}</span>
              <span>{m.mesesPagados}/{totalMeses} meses</span>
            </div>
            <div className="w-full bg-gray-700 rounded-full h-1.5 mt-2">
              <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${progress * 100}%` }} />
            </div>
          </div>
        );
      })}
