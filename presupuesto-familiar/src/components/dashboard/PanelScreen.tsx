...
import { useConfig, useMovimientos, useDeudas, useAhorros, useTarjetas, useRecurrentes, useBancos } from '@/lib/hooks';
  ...
  return (
    <div className="space-y-4 pb-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <StatCard label="Ingresos" value={formatMXN(income)} color="text-emerald-400" />
        <StatCard label="Gastos" value={formatMXN(expenses)} color="text-red-400" />
        <StatCard label="Balance" value={formatMXN(balance)} color={balance >= 0 ? 'text-emerald-400' : 'text-red-400'} />
        <StatCard label="Tasa Ahorro" value={`${(savingsRate * 100).toFixed(1)}%`} color="text-blue-400" />
      </div>

      {/* Alert Badges */}
      {(overdueRecurrentes.length > 0 || upcomingRecurrentes.length > 0) && (
